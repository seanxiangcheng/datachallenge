---
title: "Simulation and Correlation"
...
## Introduction
Revenue growth prediction is an interesting and practical problem. I do have questions because I am new to the real-business revenue prediction, but I will make some assumptions due to limitted time. In real working environment, we need to communciate to make decisions about the assumptions and parameters.

#### Assumptions
  1. The question we want to answer is the dependence of total revenue grwoth on correlation.
  ...
  3. Both features boost the revenue cumulatively (after correlation);
    * Product with more features has higher price;
    * Each feature can generate regular cash flow;
  4. Both feature is unlikely to decrease revenue.
    * Negative random variables have very small probability;
...
r.max = 0.8
```
2 functions to generate correlated random normal variables and to examine the mean and correlation matrix.
```{r, warning=FALSE}
...
Quick check the functions
```{r, warning=FALSE}
check_mean_cor(normal_xy(r=0.5, n=1e5))
```

Here, we simulate the revenue and plot the 90% confidence interval of the growth rate with both features. The standard deviations are 0.02 and 0.02.
...
In a similar manner, we can also get the revenue prediction of uniformly distributed rates. Instead of using a build-in function, we can use cholesky decomposition to transform independent random variables to correlated ones. (Functions are hidden in raw R markdown file by setting 'echo=FALSE'.)
...
The result looks weired. It may be due to incorrect correlation transform using Cholesky deposition. 
...
Test the correlation between the 2 samples.
```{r}
check_mean_cor(normal_xy(r=0.5, n=1e5))
```{r}
...
Note:
  ...
  In my research, we use advanced Monte Carlo algorithms, complex networks, and C++ to simulate physical systems. Mostly, we use independent random variables, so I don't have much experience in generating correlated random variables using R. There may be better ways to do, and I am very intestered to learn. I also hope I can learn new things from a job.

  2. **Programming language**:
  I can use R, Python, and C++. R is chosen here to be consistent with the guide R markdown file. I also have experience in interpreting outputs from SAS, STATA, and SPSS. The diverse skill set may make team-work easier.  